...
to-report energy-cost
  report  (speed / 50) ^ 3 * (creature_size / 50) ^ 2 * smell / 50
end

to-report is-home
  report (max-pxcor - xcor < 2 * speed / 100 or max-pycor - ycor < 2 * speed / 100 or xcor - min-pxcor < 2 * speed / 100 or ycor - min-pycor < 2 * speed / 100)
end

to setup
  clear-all
  set-default-shape creatures "bug"
  set-default-shape food "dot"
  ;; randomly distribute wood chips
  make-food number-food
  make-creatures number-creatures
  reset-ticks
end

to go  ;; turtle procedure
  if ticks != 0 and ticks mod day-length = 0 [ end-day ]
  ask creatures [
    ifelse eating > 0
    [
      set eating eating - 1
    ...
    [
       eat-food
    ifelse ticks mod day-length < day-length / 2
    [
      search-for-food
    ]
    [
        ifelse (energy - energy-cost) < 1
        [
        search-for-food
        ]
        [
  ...
  ask patches
  [
    if ticks mod day-length = 1
    [
      set pcolor black
    ]
    if ticks mod day-length = (day-length / 2) + 1
    [
      set pcolor 1
    ]
  ]
...
to end-day
  ...
  [
    ...
    if energy < 0 or not is-home
    [
      die
    ]
    if energy > energy-cost and is-home
    [
      hatch-creature
    ]
    set energy 0
...
to search-for-food ;; shepherds procedure
  if goal_food = nobody and any? food in-radius smell
  [
  ;;  let goal_food min-one-of (patches in-radius 25 with [pcolor = 9.9]) [distance myself]
    set goal_food min-one-of (food in-radius smell)[distance myself]
  ]
  if goal_food != nobody
  [
    set heading towards goal_food
  ]
end

to eat-food ;; shepherds procedure
  if count food-here > 0
  [
    ask one-of food-here [die]
    set energy energy + 1
    set eating (50 ^ 2) / (creature_size)
  ]
  if goal_food != nobody and distance goal_food < speed / 100
  [
    ask goal_food [die]
    set energy energy + 1
    set eating (50 ^ 2) / (creature_size)
  ...
  hatch 1
      [
        ...
        [
         ...
         ifelse (random 2 = 0)
        [
          set creature_size creature_size * 1.05
        ]
        [
          if creature_size > 1
          [
            set creature_size creature_size * 0.95
          ]
        ]
         ifelse (random 2 = 0)
        [
          set smell smell * 1.05
        ]
        [
          if smell > 1
          [
            set smell smell * 0.95
          ]
        ]
        set color palette:scale-gradient [[0 255 0] [255 0 0]] speed 30 100;(max [speed] of creatures)
        set size 5 * creature_size / 30
        set goal_food nobody
        set energy 0
        set eating 0
      ]
end
...
to make-creatures [ number ]
  create-creatures number [
    ifelse (random 2 = 0)
        [ setxy max-pxcor random-ycor ]
        [ setxy random-xcor max-pycor ]
    set color green
    set creature_size 60
    set size 5 * creature_size / 30
    set smell 50
    set goal_food nobody
    set energy 0
...
true
false
"set-plot-x-range 0 100\nset-plot-y-range 0 count creatures / 2\nset-plot-pen-mode 1                    \nset-histogram-num-bars 50" ""
PENS
"default" 1.0 0 -5298144 true "" "histogram [speed] of creatures"
...
"" ""
PENS
"default" 1.0 0 -5298144 true "" "plot mean [speed] of creatures\n"
...
"" ""
PENS
"default" 1.0 0 -13791810 true "" "plot mean [creature_size] of creatures"
...
"set-plot-x-range 0 100\nset-plot-y-range 0 count creatures / 2\nset-plot-pen-mode 1                    \nset-histogram-num-bars 50" ""
PENS
"default" 1.0 0 -13791810 true "" "histogram [creature_size] of creatures"
...
"" ""
PENS
"default" 1.0 0 -4079321 true "" "plot mean [smell] of creatures"
...
"set-plot-x-range 0 100\nset-plot-y-range 0 count creatures / 2\nset-plot-pen-mode 1                    \nset-histogram-num-bars 50" ""
PENS
"default" 1.0 0 -7171555 true "" "histogram [smell] of creatures"
